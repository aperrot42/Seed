...
int main( int argc, char *argv[] )
{
  if( argc < 4 )
    ...
    std::cerr << " nucleiImage(itkimg) membraneImage(itkimg) ";
    std::cerr << "outputImagefile(itkimg) outputSeeds(txt)" << std::endl;
    return EXIT_FAILURE;
    }
  ...
  Starting like that you ll never be efficient (especially on really short period)
      split in very small piece of code
      1 main per step
      then one you validate one step you go to the next one
      then at the end you combien all of them (if you have time)
        it will also help you to analyze all the step of your method (at least theoretically)

        1 Kishore filter (dont need qs of now)
        2 Threshold
        3 Fqst marching (possible filtering)
        4 local maximq
        5 morpho

  //  The types of the input images are instantiated by the following lines.
  typedef itk::Image< PixelType, Dimension >  InputImageType;
  typedef itk::Image< PixelType, Dimension >  OuImageType;
 ...
 typedef itk::RegularStepGradientDescentOptimizer       OptimizerType;
  ...
  // to check the result of the registration process.
  typedef itk::CheckerBoardImageFilter<FixedImageType> checkerBoardImageFilterType;
  ...
  checkerBoard->SetInput (0, fixedImageReader->GetOutput());
  checkerBoard->SetInput (1, resampler->GetOutput() );
  checkerBoard->Update();
  ...
  // we rescale the image prior to saving it
  // for most file formats compatibility
  ...
  intensityRescaler->SetInput( checkerBoard->GetOutput() );
